...
export default function Viewcube({isOrthogonal, setOrthogonal}:ViewcubeProps) {
  ...
  return createPortal(
    <>
      ...
      >
        {[...Array(6)].map((_, index) => (
          <meshStandardMaterial attachArray="material" key={index} color={hover === index ? '#23ABD5' : 'white'} />
        ))}
        <boxGeometry args={[8, 8, 8]} />
      </mesh>
      <ambientLight intensity={0.3} />
      <pointLight position={[10, 10, 10]} intensity={2} color="#23ABD5"/>
      <pointLight position={[1, 10, 1]} intensity={2} color="#23ABD5"/>
    </>,
    virtualScene
  ) as ReactElement
}
